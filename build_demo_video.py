...
SCENES: List[Scene] = [
    Scene(
        ...
        subtitle="Snowflake-Native Decision Agent",
        narration=(
            "DistrictPilot AI는 서초, 영등포, 중구를 대상으로 다음 달 어디에 어떤 렌탈 상품과 마케팅 예산을 얼마나 배분해야 하는지 "
            "답하는 스노우플레이크 네이티브 의사결정 에이전트입니다. 핵심은 단순 예측이 아니라, 예측 결과를 실제 배분 액션으로 연결하는 것입니다."
        ),
        bullets=[
            "문제: 다음 달 어떤 구에 예산을 더 배분해야 하는가",
            "대상: 서초구 · 영등포구 · 중구",
            "결과: 예측 + 근거 + 리스크 + 실행 액션",
        ],
        visual="title",
    ...
    Scene(
        ...
        subtitle="다음 달 배분 추천과 Actual vs Forecast 오버레이",
        narration=(
            "Allocation 탭에서는 다음 달 배분 비율을 바로 보여줍니다. "
    ...
    Scene(
        ...
        narration=(
            ...
            "유동인구와 카드소비, 순이동, 자산지표를 보고, 20대에서 39세 비중과 시니어 비중, 관광수요 지수와 상권 안정도를 함께 보여줍니다. "
    ...
    Scene(
        ...
        narration=(
            "AI Agent 탭에서는 사용자가 한국어로 질문하면 추천 지역, 배분 비율, 근거, 리스크, 다음 액션을 구조화된 형태로 반환합니다. "
    ...
    Scene(
        ...
        narration=(
            "Simulation 탭에서는 예산 총액과 지역별 배분을 사용자가 직접 조정할 수 있습니다. "
    ...
    Scene(
        key="11_impacts",
        title="One Engine, Two Impacts",
        subtitle="민간 의사결정과 공공 배분에 동시에 적용",
        narration=(
            "같은 엔진은 민간과 공공 양쪽에서 재사용할 수 있습니다. "
            "민간에서는 렌탈과 마케팅 예산 배분, 출점 우선순위, 설치 인력 배치에 쓰고, "
            "공공에서는 상권 활성화 예산 배분, 소상공인 지원, 관광 인프라와 현장 점검 우선순위에 확장할 수 있습니다."
        ),
        bullets=[
            "민간: 예산 배분 · 출점 · 인력배치",
            "공공: 상권활성화 · 생활SOC · 점검우선순위",
    ...
    Scene(
        ...
        narration=(
            ...
            "최종 제출에서는 사전 점검 SQL과 라이브 호환 패치까지 포함해 데모 리스크를 낮췄습니다. "
...
def render_scene(scene: Scene) -> Path:
    ...
    if scene.visual == "title":
        ...
        draw_card(draw, (980, 220, 1380, 860), "Private")
        draw_card(draw, (1420, 220, 1820, 860), "Public")
        private_items = ["렌탈/마케팅 예산", "출점 우선순위", "설치 인력 배치", "ROI 시뮬레이션"]
        public_items = ["상권 활성화 예산", "소상공인 지원", "관광 인프라", "현장 점검 우선순위"]
        y = 320
        ...
        draw.text((1050, 540), "13_live_app_compatibility_patch.sql", font=get_font(40), fill=WHITE)
        draw.text((1050, 640), "README / PPT / Demo video aligned", font=get_font(34), fill=MUTED)
